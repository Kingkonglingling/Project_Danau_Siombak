...
export default function ChatBotSection({Send, MessageCircle}) {
    ...
    // State untuk Chatbot (sudah ada)
        const [messages, setMessages] = useState([
            {
                role: 'ai',
                text: 'Selamat datang di Panduan Wisata Danau Siombak! Saya adalah asisten virtual Anda. Silakan tanyakan apa saja tentang Danau Siombak!',
                sources: [],
            },
        ]);
        const [input, setInput] = useState('');
        const [isLoading, setIsLoading] = useState(false);useRef
    ...
    const sendMessage = async (e) => {
        ...
        try {
            ...
            let response;
            for (let i = 0; i < 3; i++) { // Exponential backoff retry logic
                try {
            ...
            let aiText = "Maaf, terjadi kesalahan dalam memproses permintaan Anda.";
            let sources = [];
            ...
            setMessages(prev => [...prev, { role: 'ai', text: 'Maaf, ada masalah koneksi. Silakan coba lagi.' }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        ...
        <section id="chatbot" className="py-16 md:py-24 bg-white">
                    <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
                        <h2 className="text-base font-semibold tracking-wider text-sky-600 uppercase text-center">Bantuan Cepat</h2>
                        ...
                        <div className="bg-gray-100 rounded-xl shadow-2xl flex flex-col h-[500px]">
                                ...
                                {isLoading && (
                                    <div className="flex justify-start">
                                        <div className="bg-white p-3 rounded-xl shadow-md rounded-tl-none text-gray-500">
                                            <svg className="animate-spin h-5 w-5 text-sky-600 inline mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            ...
                            {/* Input Form */}
                            <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-200 flex space-x-3 rounded-b-xl">
                                <input
                                    type="text"
                                    value={input}
                                    onChange={(e) => setInput(e.target.value)}
                                    placeholder="Tanyakan tentang Danau Siombak..."
                                ...
                                <button
                                    ...
                                    className={`p-3 rounded-lg text-white transition duration-200 ${
                                        input.trim() && !isLoading ? 'bg-sky-600 hover:bg-sky-700' : 'bg-gray-400 cursor-not-allowed'
                                    }`}
